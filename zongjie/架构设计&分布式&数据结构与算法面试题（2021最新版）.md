...
![图片](https://mmbiz.qpic.cn/mmbiz_png/Z2CXAHBzdZG82QbkHPGcErvv7fEgRuRobC2kJDLXYo6aQpjJaSkPfWXF4X22Nz4Zq0LwZW1phsPDmtq3UaujTw/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![图片](data:image/gif;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVQImWNgYGBgAAAABQABh6FO1AAAAABJRU5ErkJggg==)二分查找也称折半查找（Binary Search），它是一种效率较高的查找方法，前提是**数据结构必须先排好序**，时间复杂度可以表示O(h)=O(log2n)，以2为底，n的对数。其缺点是要求待查表为有序表，且插入删除困难。
